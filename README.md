# John M. Kinsky GitHub Repo

You can find examples of my current and past work and some things I'm thinking about working on.

## Book

I'm currently working on a book, more properly a general guide at present, on creating and maintaining technical documentation. I plan to cover content strategy, content structure and governance, publishing, task analysis, and analytics (and probably other topics too). I work on adding content as time and family life allow. It's very much work in progress; see my progress at [Technical Documentation: From Basics to Complex Systems](https://johnkinsky.github.io/book/).

Once finished, the guide is intended to provide useful practices, methodologies, and tools to anyone who wants to create content or documentation, reduce publishing and review friction, understand effective content metrics, and track content effectiveness in real time.

### Programming Samples

I'm slowly working on some simple samples.

- [Page Analyzer Sample](https://github.com/johnkinsky/book/tree/main/page_analyzer)
- [PyPi Package Data Graph](https://github.com/johnkinsky/book/tree/main/pypi_downloads)

### Writing Portfolio
- [PDF Version](https://github.com/johnkinsky/book/blob/main/writing_samples.pdf)

## About Me

I've occupied roles in my career. At various times and at several companies, I've been a...

- Technical Writer / Information Developer
- Content Strategist
- Data Analyst
- Business Intelligence Analyst
- Web Developer and Web Designer
- Application and Sample Developer
- SEO Content Optimizer
- Information System Manager
- Engineering / Operations Manager
- People Manager (small and large teams)
- Mayor

As obe might expect, I've used many tools, technologies, and methodologies in these roles. I've listed the [tools and technologies](tools.md) that I've used extensively in the past 10 years. (The list would be nearly endless if I added everything.)

### Helpful Links

- [Helpful Resources](resources.md)
